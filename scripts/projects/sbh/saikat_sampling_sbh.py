...
E0 = 0.02                              # Energy Bias (au)
DiaCoup = 0.001                        # Diabatic Coupling (au)
WMAX = 3100.0/cminv_to_au              # Maximum Frequency (cm-1)
...
#shutil.rmtree(r'/home/smukherjee/PYTHON/TRAJECTORIES*')
...
for j in range(len(index)):
    ...
    os.system('cp configuration.inp TRAJ_INSIDE/TRAJ'+str(j+1))
    os.system('cp -r JOB_NAD TRAJ_INSIDE/TRAJ'+str(j+1))
    os.system('cp submit_nx.job TRAJ_INSIDE/TRAJ'+str(j+1))
    os.chdir('TRAJ_INSIDE/TRAJ'+str(j+1))
    with open('geom.orig', 'w') as g, open('veloc.orig', 'w') as v:
        for i in range(NB):
            g.write("{:5} {:15.10f} {:15.10f} {:15.10f} {:15.10f} {:15.10f}".format('C', 1.0, Rselect[i,j], 0.0, 0.0, Mass/amu_to_au))
            g.write('\n')
