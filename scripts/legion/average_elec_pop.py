...
def read_trajs(N):
    pops = {}
    if os.path.isfile('TRAJECTORIES/TRAJ1/elec_pop.dat'):
        for i in range(N):
            pops[i] = pd.read_table('TRAJECTORIES/TRAJ'+str(i+1)+'/elec_pop.dat', 
                                    delimiter=r'\s+', 
                                    index_col=0, 
    ...
    avrg = pd.DataFrame()
    avrg['S0_population'] = pop.xs('state 1', axis=1, level=1, drop_level=False).mean(axis=1)
    ...
    avrg.to_csv('populations.dat', sep='\t')
